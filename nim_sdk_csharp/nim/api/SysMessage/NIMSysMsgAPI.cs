...
namespace NIM.SysMessage
{
    ...
    public delegate void SendSysMsgResult(MessageArc arc);
    ...
    public class SysMsgAPI
    {
        ...
        static SysMsgAPI()
        {
            SysMsgNativeMethods.nim_sysmsg_reg_custom_notification_arc_cb(null, OnSendMsgCompleted, IntPtr.Zero);
        ...
        private static readonly CustomSysMsgArcDelegate OnSendMsgCompleted = (result, ptr) =>
        {
            if (SendSysMsgHandler != null)
            {
                ...
                if (!string.IsNullOrEmpty(result))
                {
                    var msg = MessageArc.Deserialize(result);
